...
import { MapPin, Phone, Package, Check, Truck, AlertCircle } from "lucide-react";
            ...
            <CardContent className="p-4 space-y-3">
              ...
              <div className="grid sm:grid-cols-2 gap-3 text-sm">
                <div className="flex items-start gap-2">
                  ...
                  <span>{d.pemesanan?.pelanggan?.alamat1}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Phone className="h-4 w-4 text-brown-500 flex-shrink-0" />
                  <span>{d.pemesanan?.pelanggan?.telepon}</span>
                </div>
              </div>
              <div className="pt-2 flex gap-2">
                {d.status_kirim === "Menunggu_Kurir" && (
                  <Button 
                    size="sm" 
                    onClick={() => updateStatus(d.id, "Sedang_Dikirim")} 
                ...
                {d.status_kirim === "Sedang_Dikirim" && (
                  <Button 
                    size="sm" 
                    onClick={() => updateStatus(d.id, "Tiba_Ditujuan")} 
                    disabled={updating === d.id}
